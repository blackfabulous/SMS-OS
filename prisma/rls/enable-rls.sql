...
--   3. Every tenant-owned table now carries its own `schoolId` column. Rows whose
--      `schoolId` is NULL fail closed; `AuditLog` and `Outbox` keep the column
--      nullable so system-level events can be stored, but all business rows
--      must set it.
