...
| Tests | 13 Vitest files, including `tests/tenant-safety.test.ts`, `tests/tenant-context.test.ts`, `tests/finance-scope.test.ts`, `tests/student-access.test.ts`; Playwright E2E (41 tests across chromium + Mobile Chrome) covers public pages, contact submission, admissions apply, authenticated dashboard/module loads, student search, finance dashboard, and `/api/health/deep` | Partial |
