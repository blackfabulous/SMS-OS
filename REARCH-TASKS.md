...
| Service layer (`src/server`) | `src/lib/settings.ts`, `src/server/finance/scope.ts`, `src/server/services/payment-service.ts`, `src/server/services/notifications.ts`, `src/lib/finance-calc.ts`, `src/lib/grading.ts`, etc. | Partial — payments + notification services moved; other contexts pending |
