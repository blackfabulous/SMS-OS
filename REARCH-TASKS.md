...
| Tests | 13 Vitest files, including `tests/tenant-safety.test.ts`, `tests/tenant-context.test.ts`, `tests/finance-scope.test.ts`, `tests/student-access.test.ts`; Playwright E2E (29 tests across chromium + Mobile Chrome) covers public pages, contact submission, admissions apply, authenticated dashboard/module loads, student search, finance dashboard, and `/api/health/deep` | Partial |
...
| RA-D3 | **Module decomposition standard** — `page.tsx (server) + components/ + hooks/`, ≤~300 lines/file (current modules are 800–1,500). Pilot completed for `timetable-module.tsx` (`src/components/modules/timetable/` with constants, types, dialog, grid components); remaining modules pending | P2 | XL | RA-D2 | No monolith module files |
