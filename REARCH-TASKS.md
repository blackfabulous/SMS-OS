...
| Tests | 13 Vitest files, including `tests/tenant-safety.test.ts`, `tests/tenant-context.test.ts`, `tests/finance-scope.test.ts`, `tests/student-access.test.ts`; Playwright E2E (119 tests across chromium + Mobile Chrome) covering all 39 dashboard modules, public pages, contact submission, admissions apply, student search, finance dashboard, and `/api/health/deep` | Partial |
