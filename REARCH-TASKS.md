...
| TanStack Query | `QueryProvider` added to root layout; `src/lib/api-client.ts` typed envelope client; `src/hooks/use-api-query.ts` hooks; dashboard, `SettingsRegistryPanel`, and `alumni-module` migrated to `useApiQuery`/`useApiMutation`; other modules pending | Partial |
...
| RA-C3 | **Response envelope** `{data}` / `{error:{code,message,details}}` + `ok()`/`fail()` helpers in `src/server/http.ts` | P2 | S | — | Helpers created; `/api/finance/payments`, `/api/notifications/send`, `/api/attendance`, `/api/dashboard`, `/api/finance` (GET), and `/api/settings` converted; route-wide rollout pending |
